...
/// A common error type to be used throughout the API.
...
/// Axum allows to return `Result` from handler functions, but the error type
...
impl IntoResponse for Error {
    fn into_response(self) -> Response {
        match self {
            ...
            Self::Anyhow(ref e) => {
                // Tracing gets linked to the HTTP request by `TraceLayer`.
